...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/05/31 15:57:57 by masebast          #+#    #+#             */
/*   Updated: 2022/06/22 16:43:39 by masebast         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	*ft_transformer(int *ar, int size)
{
	int	*temp;
	int	*new;
	int	i;
...
int	main(int argc, char *argv[])
{
	...
	if (ft_checkfordoubles(data.ar_a, data.size_a) == 0)
		ft_error();
	data.ar_a = ft_transformer(data.ar_a, data.size_a);
	ft_resolve(&data);
	free(data.ar_a);
	free(data.ar_b);
	return (0);
